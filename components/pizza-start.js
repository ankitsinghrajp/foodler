...
import pizzaStart from "../data/pizzaStart.json";
import { toast } from "sonner";

const PizzaStart = () => {
  const [cartState, setCartState] = useState(
    pizzaStart.map((pizza) => ({
      name: pizza.name,
  ...
  const handleAddToCart = (index) => {
    ...
    const original = pizzaStart[index];
  ...
  return (
    <div className="pt-20 pb-10">
      <div className="flex justify-center items-center border-b-2 pb-4 border-dotted">
        <div>
          <h3 className="font-bold text-center text-3xl text-green-500">
            Pizza Start
          </h3>
